...
public class MainActivity extends ViewPagerActivity {
  ...
  public void onCreate(Bundle savedInstanceState) {
    ...
    mSectionsPagerAdapter.addSection(mScanView, "Lista de nodos");
    mSectionsPagerAdapter.addSection(mMapView, "Mapa");
  ...
  public void onMapViewReady(View view) {
	    // Initial state of widgets
  ...
  private void startScan() {
    ...
    if (mBleSupported) {
      ...
      if (!mScanning) {
        setError("No se pueden descubrir nodos");
        setBusy(false);
      }
    } else {
      setError("BLE no está soportado por este dispositivo");
    }
  ...
  public void updateGuiState() {
    ...
    if (mBtEnabled) {
      if (mScanning) {
        // BLE Host connected
        if (mConnIndex != NO_DEVICE) {
          String txt = mBluetoothDevice.getName() + " conectado";
          mScanView.setStatus(txt);
        } else {
          mScanView.setStatus(mNumDevs + " nodos");
        }
      }
    } else {
      mDeviceInfoList.clear();
  		...
  		if (BluetoothAdapter.ACTION_STATE_CHANGED.equals(action)) {
  	      ...
  	      Toast.makeText(context, R.string.app_closing, Toast.LENGTH_LONG).show();
  			...
  			if (status == BluetoothGatt.GATT_SUCCESS) {
  				...
  				setError("Conexión fallida. Estado: " + status);
  			...
  			if (status == BluetoothGatt.GATT_SUCCESS) {
  				...
  				setError("Desconexión fallida. Estado: " + status);  				
  			}
  			mConnIndex = NO_DEVICE;
  			mBluetoothLeService.close();
  		} else {
